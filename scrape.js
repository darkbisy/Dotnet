...
const proxies = [];
const output_file = 'hellooom.txt';
...
async function fetchProxies() {
  for (const site of raw_proxy_sites) {
    try {
      ...
      console.error(`Failed to fetch proxies from ${site}: ${error.message}`);
    }
  }

  fs.writeFileSync(output_file, proxies.join('\n'));
  console.log(`Proxies successfully fetched and saved in ${output_file}`);
}
